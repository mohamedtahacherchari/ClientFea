...
import { addApprendreApi } from "../../redux/actions/apprendre/apprendreActions";
...
const AddApprendre = ({addApprendreApi, error}) => {
    ...
    const uploadFileHandler = async (apprendre, fileType) => {
      const file = apprendre.target.files[0];
...
function mapStateToProps({apprendre, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addApprendreApi})(AddApprendre)