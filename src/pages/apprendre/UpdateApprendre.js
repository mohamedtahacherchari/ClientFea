...
import { ShowApprendresApi, updateApprendreApi,listApprendreDetails} from "../../redux/actions/apprendre/apprendreActions";
...
import { APPRENDRE_UPDATE_RESET } from '../../redux/actions/servantActions/constant/constantZervant/apprendreConstant';
...
const UpdateApprendre = ({ updateApprendreApi}) => {
  const{id} =useParams()
  //const event = useSelector((state) => state.event);
  const apprendres = useSelector((state) => state.apprendres);
  const apprendreDetails = useSelector((state) => state.apprendreDetails)
  const { loading, error,apprendre } = apprendreDetails
  console.log(apprendres)
  const apprendreUpdate = useSelector((state) => state.apprendreUpdate)
  const {
 ...
 // console.log(event,"looool")
  ...
  const [selectedApprendre, setSelectedApprendre] = useState(null);
  ...
  useEffect(() => {
    if (successUpdate) {
      ...
      refreshPage()
    dispatch({ type: APPRENDRE_UPDATE_RESET})
  }
    if (!apprendre.title ||apprendre._id !== id){
      dispatch(listApprendreDetails(id));

    } 
    else{
// Initialisation des valeurs par défaut pour le formulaire
     setValue('isActive',apprendre.isActive);
     setValue('title', apprendre.title || "");
     setValue('fonction', apprendre.fonction || "");
   //  setValue('describe', education.describe || "Aucune description fournie");
     setValue('imageUrl', apprendre.imageUrl || "");
     setValue('pdfUrl', apprendre.pdfUrl || "");
   //  setValue('pdfUrl2', apprendre.pdfUrl2 || "");
     setValue('selectedClients', apprendre.selectedClients || []);
     setSelectedClients(apprendre.selectedClients || []);

    }

  }, [dispatch, id,apprendre,setValue,listApprendreDetails])
  ...
  const onSubmit2 = async (data) => {
    ...
    await  dispatch(updateApprendreApi(data, id, token));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Utilisez dispatch pour appeler l'action updateAudiovisuelApi
      await updateApprendreApi(data, id, token);
      
      // Remplacer navigate par une redirection avec window.location.href
      window.location.href = "/inv/MyComponentReplay8";
    } catch (err) {
      setError(true);
      console.error("Failed to update apprendre:", err);
    }
};
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateApprendre);
