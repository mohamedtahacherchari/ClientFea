...
import { ShowSadaqasApi, updateSadaqaApi,listSadaqaDetails} from "../../redux/actions/sadaqa/sadaqaActions";
...
import { SADAQA_UPDATE_RESET } from '../../redux/actions/servantActions/constant/constantZervant/sadaqaConstant';
...
const UpdateSadaqa = ({ updateSadaqaApi}) => {
  const{id} =useParams()
  //const event = useSelector((state) => state.event);
  const sadaqas = useSelector((state) => state.sadaqas);
  const sadaqaDetails = useSelector((state) => state.sadaqaDetails)
  const { loading, error,sadaqa } = sadaqaDetails
  console.log(sadaqas)
  const sadaqaUpdate = useSelector((state) => state.sadaqaUpdate)
  const {
 ...
 // console.log(event,"looool")
  ...
  const [selectedSadaqa, setSelectedSadaqa] = useState(null);
  ...
  useEffect(() => {
    if (successUpdate) {
      ...
      refreshPage()
    dispatch({ type: SADAQA_UPDATE_RESET})
  }
    if (!sadaqa.title ||sadaqa._id !== id){
      dispatch(listSadaqaDetails(id));

    } 
    else{
// Initialisation des valeurs par défaut pour le formulaire
     setValue('isActive',sadaqa.isActive);
     setValue('title', sadaqa.title || "");
     setValue('fonction', sadaqa.fonction || "");
   //  setValue('describe', education.describe || "Aucune description fournie");
     setValue('imageUrl', sadaqa.imageUrl || "");
     setValue('pdfUrl', sadaqa.pdfUrl || "");
     setValue('pdfUrl2', sadaqa.pdfUrl2 || "");
     setValue('pdfUrl3', sadaqa.pdfUrl3 || "");
     setValue('youtubeLink', sadaqa.youtubeLink || "");
     setValue('selectedClients', sadaqa.selectedClients || []);
     setSelectedClients(sadaqa.selectedClients || []);

    }

  }, [dispatch, id,sadaqa,setValue,listSadaqaDetails])
  ...
  />
    ...
    <div className="mb-4">
      <label htmlFor="title" className="form-label">Mettre à jour la description de l'équipe </label>
...
<div className="mb-4">
        <label htmlFor="youtubeLink" className="form-label">Mettre à jour  le lien YouTube des Sprints Sadaqa</label>
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf'>Mettre à jour le Sprint 'FEA SADAQA' sous forme de PDF</FormLabel>
    <TextField
        id='pdf2'
        type='text'
        placeholder='Enter PDF url'
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf'>Mettre à jour les Certificats de remerciement sous forme pdf</FormLabel>
    <TextField
        id='pdf3'
        type='text'
        placeholder='Enter PDF url'
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateSadaqa);
