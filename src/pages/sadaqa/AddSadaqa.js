...
import { addSadaqaApi } from "../../redux/actions/sadaqa/sadaqaActions";
...
const AddSadaqa = ({addSadaqaApi, error}) => {
    ...
    const uploadFileHandler = async (sadaqa, fileType) => {
      const file = sadaqa.target.files[0];
      ...
      try {
          ...
          switch (fileType) {
              ...
              case 'pdf2':
                  ...
                  break;
                  case 'pdf3':
                    setValue("pdfUrl3", `\\uploads/${data.pdfUrl3}`); // Modifiez l'URL de la vidéo pour commencer par '\uploads'
                    break;
                  case 'video2':
  ...
  />
      ...
      <label htmlFor="title" className="form-label">Ajouter une déscription à l'équipe </label>
...
<div className="mb-4">
        <label htmlFor="youtubeLink" className="form-label">Ajouter le lien YouTube des Sprints Sadaqa</label>
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf2'>Ajouter le Sprint 'FEA SADAQA' sous forme de PDF</FormLabel>
    <TextField
        id='pdf2'
        type='text'
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf3'>Ajouter les certificats de remerciement sous forme de PDF.</FormLabel>
    <TextField
        id='pdf3'
        type='text'
...
function mapStateToProps({sadaqa, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addSadaqaApi})(AddSadaqa)