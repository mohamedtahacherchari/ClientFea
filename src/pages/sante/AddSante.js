...
import { addSanteApi } from "../../redux/actions/sante/santeActions";
...
const AddSante = ({addSanteApi, error}) => {
    ...
    const uploadFileHandler = async (sante, fileType) => {
      const file = sante.target.files[0];
...
function mapStateToProps({sante, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addSanteApi})(AddSante)