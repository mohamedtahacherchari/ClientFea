...
import { addEntraideApi } from "../../redux/actions/entraide/entraideActions";
...
const AddEntraide = ({addEntraideApi, error}) => {
    ...
    const uploadFileHandler = async (entraide, fileType) => {
      const file = entraide.target.files[0];
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf2'>Ajouter Success stories sous forme pdf</FormLabel>
    <TextField
        id='pdf2'
        type='text'
...
function mapStateToProps({entraide, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addEntraideApi})(AddEntraide)