...
import { addVoyageApi } from "../../redux/actions/voyage/voyageActions";
...
const AddVoyage = ({addVoyageApi, error}) => {
    ...
    const uploadFileHandler = async (voyage, fileType) => {
      const file = voyage.target.files[0];
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf'>Ajoutez le plan sous forme pdf</FormLabel>
    <TextField
        id='pdf'
        type='text'
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf2'>Ajoutez Les 12 pays que nous 
...
<FormControl fullWidth>
    <FormLabel htmlFor='pdf3'>
      Ajoutez sites que nous recommandons 
    </FormLabel>
    <TextField
        id='pdf3'
        type='text'
...
function mapStateToProps({voyage, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addVoyageApi})(AddVoyage)