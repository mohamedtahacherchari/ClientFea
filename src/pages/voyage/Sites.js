...
const Sites = ({ ShowVoyagesApi }) => {
  ...
  useEffect(() => {
    if (firstActiveVoyage && firstActiveVoyage.pdfUrl3) {
      const fetchPDF = async () => {
        try {
          const response = await fetch(firstActiveVoyage.pdfUrl3);
...
export default connect(null, { ShowVoyagesApi })(Sites);
