...
import { ShowInvestirsApi, updateInvestirApi,listInvestirDetails} from "../../redux/actions/investir/investirActions";
...
import { INVESTIR_UPDATE_RESET } from '../../redux/actions/servantActions/constant/constantZervant/investirConstant';
...
const UpdateInvestir = ({ updateInvestirApi}) => {
  const{id} =useParams()
  //const event = useSelector((state) => state.event);
  const investirs = useSelector((state) => state.investirs);
  const investirDetails = useSelector((state) => state.investirDetails)
  const { loading, error,investir } = investirDetails
  console.log(investirs)
  const investirUpdate = useSelector((state) => state.investirUpdate)
  const {
 ...
 // console.log(event,"looool")
  ...
  const [selectedInvestir, setSelectedInvestir] = useState(null);
  ...
  useEffect(() => {
    if (successUpdate) {
      ...
      refreshPage()
    dispatch({ type: INVESTIR_UPDATE_RESET})
  }
    if (!investir.title ||investir._id !== id){
      dispatch(listInvestirDetails(id));

    } 
    else{
// Initialisation des valeurs par défaut pour le formulaire
     setValue('isActive',investir.isActive);
     setValue('title', investir.title || "");
     setValue('fonction', investir.fonction || "");
   //  setValue('describe', education.describe || "Aucune description fournie");
     setValue('imageUrl', investir.imageUrl || "");
     setValue('pdfUrl', investir.pdfUrl || "");
     setValue('pdfUrl2', investir.pdfUrl2 || "");
     setValue('selectedClients', investir.selectedClients || []);
     setSelectedClients(investir.selectedClients || []);

    }

  }, [dispatch, id,investir,setValue,listInvestirDetails])
  ...
  const onSubmit2 = async (data) => {
    ...
    await  dispatch(updateInvestirApi(data, id, token));
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Utilisez dispatch pour appeler l'action updateAudiovisuelApi
      await updateInvestirApi(data, id, token);
      
      // Remplacer navigate par une redirection avec window.location.href
      window.location.href = "/inv/MyComponentReplay7";
    } catch (err) {
      setError(true);
      console.error("Failed to update investir:", err);
    }
};
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateInvestir);
