...
const InvestirCard = ({ imageUrl, name, description, fonction }) => {
  const cardStyle = {
    width: '500px',
    height: '700px',
    display: 'flex',
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={titleStyle}>{name}Investir Pro</div>
      <div style={subtitleStyle}>{fonction}</div>
      <div style={descriptionStyle}>
        Très expérimenté associativement,
        <strong><u>{description}</u></strong> 
        s'occupe des Grands investissements . Sous sa coordination les
