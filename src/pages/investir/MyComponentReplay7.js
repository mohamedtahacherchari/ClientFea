...
import { ShowInvestirsApi ,updateInvestirStatus} from "../../redux/actions/investir/investirActions";
...
const MyComponentReplay = ({ ShowInvestirsApi }) => {
 ...
 /* const educations = useSelector((state) => {
  ...
  const investirStatus = useSelector(state => state.investirStatus);
console.log(investirs)
  ...
  console.log(investirs,"hello evenyt")
  ...
  const handleStatusChange = async (id, isActive) => {
    try {
      if (isActive) {
        ...
        if (activeInvestirs.length > 0) {
          alert("Un autre investir est déjà actif. Veuillez désactiver l'autre avant d'activer celui-ci.");
          return;
        }
  
        // Désactiver tous les autres educations avant d'activer le nouveau
        await Promise.all(
          investirs.filter(investir => investir._id !== id && investir.isActive)
            .map(investir => dispatch(updateInvestirStatus(investir._id, false)))
        );
      }
  
      // Activer ou désactiver l'education actuel
      await dispatch(updateInvestirStatus(id, isActive));
  ...
  const handleDelete2 = async (investir, id) => {
    investir.stopPropagation();
  
    try {
      if (investirs._id !== id) {
        if (window.confirm("Are you sure you want to delete this Investissement?")) {
          setLoading(true);
          await axios.delete(`http://localhost:5000/api/investirs/${id}/delete`, {
  ...
  return (
    ...
    <IconButton
      ...
      >
        Ajouter Ivestissement
        <AddIcon />
      </IconButton>
      <List>
  {investirs.map((investir) => (
    <ListItem
      key={investir._id}
      ...
      disableRipple
      component={Link}
      to={'/inv/AppNavbarInvestir'}
    >
      <ListItemText
        primary={investir.title}
        secondary={
          <>
            <span>{investir.type}</span><br />
            <span>{investir.describe}</span><br/>
          </>
        }
      />
      <img src={investir.imageUrl} alt="Investissement Icon" className={classes.image} />
      ...
      control={
        <Switch
          checked={investir.isActive}
          onChange={() => handleStatusChange(investir.id, !investir.isActive)}
          color="primary"
        />
      }
      label={
        <span style={{ color: investir.isActive ? 'green' : 'red' }}>
          {investir.isActive ? 'Active' : 'Inactive'}
        </span>
      }
    ...
    />
      ...
      {user.role === 1 && (
        <IconButton component={Link} to={`/inv/UpdateInvestir/${investir.id}`}>
          <EditIcon />
        </IconButton>
      )}
      {user.role === 1 && (
        <IconButton onClick={(e) => handleDelete2(e, investir.id)}>
          <DeleteIcon />
        </IconButton>
      )}
    </ListItem>
...
export default connect(null, { ShowInvestirsApi })(MyComponentReplay);
