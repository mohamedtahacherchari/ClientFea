...
import { addInvestirApi } from "../../redux/actions/investir/investirActions";
...
const AddInvestir = ({addInvestirApi, error}) => {
    ...
    const uploadFileHandler = async (investir, fileType) => {
      const file = investir.target.files[0];
...
function mapStateToProps({investir, error}){
  return{
    error
    // audiovisuel
  }
...
export default connect(mapStateToProps , {addInvestirApi})(AddInvestir)